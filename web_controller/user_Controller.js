  ...
  try {
    ...
    if (result.error) {
      ...
      if (match) {
        ...
        const update_user_password = await updateUserbyPass_1(hash, new_password, user_id)
    ...
    if (result.error) {
      ...
      if (userInfo.length !== 0) {
        ...
        if (display_name_check != 0) {
          return res.json({
            success: false,
            message: "Display name is already taken. Please use a different display name.",
            status: 400,
          });
        }
        let user = {
